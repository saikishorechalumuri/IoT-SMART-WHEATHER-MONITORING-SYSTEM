...
void initialization()
{	
				EEPROM_SW_INT();  // SW1--->RTC  SW2---->EEPROM
				sys_tick_int();
...
{   
				UART1_OutString("AT\r\n");
																 //  if(UART1_instring())
																	//	LCD_WRITE_STRING("ERROR at"); //Print error message and restart the program from first line
				delaysec(1);
				...
				UART1_OutString("AT+CWJAP=\"FTTH-BSNL\",\"4132968086\"\r\n");
																//	if(UART1_instring())
																//		LCD_WRITE_STRING("ERROR pass");
				delaysec(1);
				UART1_OutString("AT+CIFSR\r\n");
																//	if(UART1_instring())
																//		 LCD_WRITE_STRING("ERROR pass1");//Print error message and restart the program from first line
				delaysec(1);
				UART1_OutString("AT+CIPMUX=0\r\n");
																	//	if(UART1_instring())
																	//	 LCD_WRITE_STRING("ERROR pass2");
				delaysec(1);
				UART1_OutString("AT+CWLAP\r\n");
																	//	if(UART1_instring())
																	//	 LCD_WRITE_STRING("ERROR pass3");
				delaysec(1);
				UART1_OutString("AT+CIPMODE=0\r\n");
																//		if(UART1_instring())
			//LCD_WRITE_STRING("ERROR pass4");//Print error message and restart the program from first line
				delaysec(1);
				LCD_WRITE_COMREG(0x01);//clear screen
				LCD_WRITE_COMREG(0x80); //SET CURSOR POSITION
}	
